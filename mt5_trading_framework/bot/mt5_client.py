...
from bot.utils import BotConfigError, timeframe_to_mt5
...
class MT5Client:
    ...
    def initialize(self) -> None:
        if not self.path:
            raise BotConfigError("MT5 terminal path is empty. Set MT5_PATH in .env")
    ...
    def login(self) -> None:
        if not self.login_id or not self.password or not self.server:
            raise BotConfigError("Missing MT5 credentials in environment")
    ...
    def validate_symbols(self, symbols) -> None:
        for sym in symbols:
            info = mt5.symbol_info(sym)
            if info is None:
    ...
    def copy_rates_range(self, symbol: str, timeframe: str, date_from, date_to):
        ...
        if rates is None:
            raise RuntimeError(f"Failed to get rates range for {symbol} {timeframe}: {mt5.last_error()}")
        return rates
    ...
    def is_spread_ok(self, symbol: str, max_spread_points: float) -> bool:
        info = self.get_symbol_info(symbol)
        tick = self.get_tick(symbol)
        spread_points = (tick.ask - tick.bid) / info.point
        if spread_points > max_spread_points:
            self.logger.debug("Spread %.2f > limit %.2f for %s", spread_points, max_spread_points, symbol)
            return False
        return True
    ...
    def normalize_volume(self, symbol_info, lot: float) -> float:
        ...
        stepped = max(min_vol, min(max_vol, (lot // step) * step))
    ...
    def is_tradeable(self, symbol: str) -> bool:
        info = self.get_symbol_info(symbol)
        return info.trade_mode in (mt5.SYMBOL_TRADE_MODE_FULL, mt5.SYMBOL_TRADE_MODE_LONGONLY, mt5.SYMBOL_TRADE_MODE_SHORTONLY)
