...
from bot.utils import ExecutionSettings, Signal


class ExecutionManager:
    def __init__(self, mt5_client, config: ExecutionSettings, logger) -> None:
    ...
    def _journal(self, symbol: str, meta: Dict[str, object], lot: float, price: float, sl: float, tp: float, result: str) -> None:
        ...
        with open(self.config.journal_csv, "a", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(
                [
                    ...
                    sl,
                    tp,
                    meta.get("spread_points", 0.0),
                    meta.get("reason", {}),
                    result,
                ]
            )

    def execute_trade(self, symbol: str, signal: Signal, tick, params: Tuple[float, float, float], meta: Dict[str, object]) -> None:
        lot, sl, tp = params
        ...
        while attempt <= self.config.retries:
            try:
                ...
                self._journal(symbol, meta, lot, price, sl, tp, result=str(result_code))
                return
        ...
        self._journal(symbol, meta, lot, price, sl, tp, result="failed")
