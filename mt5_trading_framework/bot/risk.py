...
from bot.utils import RiskSettings, Signal


class RiskManager:
    def __init__(self, risk: RiskSettings, logger) -> None:
        self.risk = risk
        self.logger = logger
    ...
    def _calc_lot(
        self, balance: float, sl_points: float, tick_value: float, lot_step: float, lot_min: float
    ) -> float:
        risk_amount = balance * self.risk.risk_per_trade
    ...
    def build_order_params(self, signal: Signal, tick, symbol_info, atr: float) -> Optional[Tuple[float, float, float]]:
        sl_points = atr * self.risk.atr_sl_mult / symbol_info.point
        tp_points = atr * self.risk.atr_tp_mult / symbol_info.point
        ...
        sl_price = price - atr * self.risk.atr_sl_mult if signal == Signal.BUY else price + atr * self.risk.atr_sl_mult
        tp_price = price + atr * self.risk.atr_tp_mult if signal == Signal.BUY else price - atr * self.risk.atr_tp_mult

        rr = self.risk.atr_tp_mult / self.risk.atr_sl_mult if self.risk.atr_sl_mult else 0
        if rr < self.risk.min_rr:
            self.logger.info("RR %.2f below min %.2f", rr, self.risk.min_rr)
            return None
    ...
    def should_halt_trading(self, mt5_client, symbol: str, now: datetime) -> Optional[str]:
        ...
        if self.risk.kill_switch_pct and self.start_equity:
            drop_pct = (self.start_equity - account.equity) / self.start_equity
            if drop_pct >= self.risk.kill_switch_pct:
        ...
        if deals:
            ...
            if daily_pl < 0 and abs(daily_pl) >= account.balance * self.risk.daily_loss_pct:
